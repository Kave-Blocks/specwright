...
Applying a change moves the build list and leaves the spec where it was, so after an apply the spec no longer describes the project. Specwright makes that **visible and countable** — "2 changes applied since Version 3" — and regenerates nothing on its own. This closes the read half of the change loop; without it the next proposal is reasoned against a spec that does not contain the work the last one added, so it re-proposes that work, and the key-collision skip swallows the duplicate as "1 unit skipped" with no hint that a stale spec is the real cause.

- **Drift is derived, never stored.** A change already records `baseSpecId`, and a spec version is only ever assigned upward, so a spec is behind exactly when an `APPLIED` change's base version is still the project's highest — had a spec been generated after that change, the current version would be higher. A `specStaleSince` column would store nothing those two numbers already say and would need keeping in sync with every apply and every generation. This is `39`'s rule a third time: record a fact already true, not one that anticipates a relationship.
- **"Current" means the highest version that still exists**, read from the spec rows — not `Project.nextSpecVersion - 1`, which records the last number *assigned* and diverges the moment a spec is removed. `currentSpecVersionOf` in `lib/changes.ts` is the single implementation, shared with the apply path's staleness check rather than written twice.
- Counted on the **spec listing route**, not a route of its own: the one surface that renders drift already calls `GET /api/projects/{projectId}/specs` on mount, so a second endpoint would double the requests to report one number — and the two could then be fetched at different moments and disagree on screen.
- Only `APPLIED` changes count. A `PROPOSED` or `DISCARDED` change has not touched the build list, so it cannot have put the spec behind.
- A project with **no spec counts zero**, not an error: there is nothing to be behind. `40` already refuses a proposal against a project with no spec, and that refusal is deliberately not widened to a stale one — blocking a proposal because the spec is behind would block the exact action that resolves it.
- Nothing regenerates automatically. Drift is reported at the two points a person can act on it — the apply outcome, where it is created, and the Specs view, beside the control that would clear it.
- **Counting the drift is not the same as being able to resolve it, and the UI must not claim otherwise.** A spec is generated from the canvas graph, the conversation, and the brief; the build list is not an input. Applying a change writes build units and leaves the canvas alone, so regenerating while a change is still unpushed produces a spec that misses the applied work *and* resets the count to zero — a worse state than the notice not existing, because the indicator then reads "resolved". Both surfaces therefore point at the **canvas** as the step that has to happen first. `43`'s canvas write-back is what makes that step an action rather than manual work; until every applied change since the current spec has been pushed, neither surface may tell anyone to regenerate.
- **The drift count and the unpushed count are two numbers, and only the first one is drift.** `countAppliedChangesSinceCurrentSpec` returns both: `appliedSinceCurrentSpec` is the drift, measured from spec versions exactly as above, and `unpushedSinceCurrentSpec` is how many of those have not reached the canvas. Pushing a change lowers the second and **never** the first — the spec really is still behind until it is rewritten, and only generating one clears it. What the second number decides is not whether the spec is behind but whether regenerating would *help*, which is the difference between the notice's two states.

## Canvas Write-Back

Applying a change moves the build list; the canvas is what a spec is written from. So until an applied change reaches the canvas, every later spec silently describes a system without the work that change added — the last structurally broken link in the change loop, and the reason `41` deliberately wrote no spec and touched no room. Pushing a change draws its architecture delta onto the canvas **through the existing design path**, and **additively only**.
...
- **It is written only after the room mutation succeeds**, so `null` doubles as the retry state and a failed push leaves the action available. The cost is stated rather than hidden: two members pushing the same change at the same instant can both pass the check and both draw, because nothing between the two systems is atomic. Claiming the push first would close that window by trading a rare duplicate for a routine unrecoverable one — a crash after the claim would mark a change drawn that was never drawn, and it could never be pushed again.
- **A change may be pushed once, and only while `APPLIED`.** Both refusals are decided by `loadPushableChange`, asked twice: by `POST /api/ai/canvas` before a run is started, so a request that cannot succeed never spends a model call (`40`'s discipline), and again inside the task, because a change can move in between. One rule, one implementation — two copies is how the two come to disagree.
- **The room a task writes into is always the one the access check resolved, never a second id the caller supplied.** `POST /api/ai/canvas` takes only `projectId` and passes `roomId: project.id`; `/api/ai/spec` and `/api/ai/change` resolve access *from* `roomId` and pass `project.id` onward. Stated generally, because this is the rule and not a detail of one route: **a route that access-checks one id and acts on another is the bug.** The design route did exactly that until 2026-08-17 ([`progress/2026-08-17-design-route-room-scoping.md`](progress/2026-08-17-design-route-room-scoping.md)) — a `projectId` the caller could reach plus a `roomId` they could not passed the check and mutated a stranger's canvas. Room ids are not secret (the room id *is* the project id, and it is in the URL bar), and the tasks mutate through the secret-key Liveblocks server client, which no room-level ACL backstops — so the handler's check is the entire boundary. Any future room-scoped AI route derives its room the same way.
