...
4. `npm run verify:db` passes, so the unit did not silently break an earlier one. It runs the five library/database suites against the real Postgres and Blob store and needs no dev server. Run it here, at the moment a unit closes, rather than on a timer — a gate that fires on every turn costs two minutes of round-trips to tell you a comment is fine, and gets disabled within a week. `npm run verify:http` is its counterpart for the three HTTP suites, but it needs `npm run dev` up and a live Clerk session, so it stays a deliberate step rather than part of this checklist.
5. The unit is committed on its own, before the next unit starts. Shared files (`lib/build-units.ts`, `prisma/models/*.prisma`) accumulate several units' work, so a per-unit commit is only possible while that unit is the newest thing in the tree — it cannot be reconstructed later. Units 38–41 are the proof: they went in as one commit because nobody split them while splitting was still possible.
