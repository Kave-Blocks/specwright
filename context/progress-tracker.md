...
| 43 | [canvas-write-back](progress/43-canvas-write-back.md) | shipped | structural | An applied change is pushed onto the canvas through the existing design path, additively; destructive operations are stripped in code and removals are reported, never drawn. 45 library + DB checks pass. Nothing that touches a Liveblocks room, the HTTP layer, or the browser has been exercised — including the end-to-end "regenerate and the spec contains the change" check this unit exists for |
| 44 | [spec-derived-build-units](progress/44-spec-derived-build-units.md) | shipped | structural | A project's current spec derives its first build units, closing the asymmetry `41` created. `38`'s producer contract implemented a second time and the first time against model output: only adds, never writes a human-owned column, and a key collision makes a second derivation a no-op — so no migration and no "already derived" column. 43 library + DB checks pass. **No UI**: nothing in the product calls `POST /api/ai/units` yet, which is the next unit. The task has never run in a worker |
...
| 46 | [derive-units-ui](feature-specs/46-derive-units-ui.md) | specced | none | The control that calls `44`'s route — deriving a project's first build units from its spec, on the Build tab. `44` shipped the whole path and stopped at the route by its own scope limit, so the capability is currently reachable over HTTP and by no person |
...
- ~~**Build units have no automatic producer for a project's *first* set.**~~ — **closed 2026-08-18 by unit [`44`](progress/44-spec-derived-build-units.md).** A project's current spec now derives the units it implies, through `38`'s producer contract implemented a second time — match on `key`, add what does not exist, never write a human-owned column. It needed no migration: `specId` and `BuildUnitSource.SPEC` had been in place since `39` and `38` respectively, waiting for exactly this producer. The reasoning is recorded in `architecture-context.md` under `## Deriving Build Units From A Spec`.
  ...
  **One thing this does not yet give anyone.** The unit deliberately stops at the route — `ai-workflow-rules.md` forbids combining UI and background-task changes in one step, and the spec path was split the same way across `27`/`28`/`29`. So **nothing in the product calls `POST /api/ai/units`**: the capability is reachable over HTTP and by `verify:units`, and by nothing a person can click. **Specced 2026-08-18 as unit [`46`](feature-specs/46-derive-units-ui.md)** — the control on the Build tab, primary in the empty state and quiet beside the add form, reporting created, skipped, and dropped separately. Close this paragraph when it ships. The task has also never run in a worker, so it shares the standing ops blocker below.

- ~~**Applying a change never redraws the canvas, and the canvas is what specs are generated from.**~~ — **structurally closed 2026-08-17 by unit [`43`](progress/43-canvas-write-back.md).** An applied change can now be pushed onto the canvas through the existing design path, additively: destructive operations are stripped from the model's plan **in code** rather than forbidden in the prompt, and `removed` deltas are reported rather than drawn. `42`'s drift notice gained the second state it was always meant to have — while any applied change is unpushed it names the step that comes first, and only once all of them are on the canvas does it say that regenerating brings the spec up to date. The reasoning is recorded in `architecture-context.md` under `## Canvas Write-Back` and in `ui-context.md` under Specs and Changes.

  **One thing this does not yet prove, and it is the check the unit exists for.** Pushing every applied change since the current spec and then generating a spec that *contains the changed architecture* has never been run — it needs a signed-in browser, a live `npx trigger dev` worker, and a real model call. Until it passes, the loop is closed in structure and unproven in fact. It is the first item under "Not verified" in `43`'s progress file, and the natural first task of the next browser pass.
